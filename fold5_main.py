...
def main():
    ...
    parser.add_argument('--logger_file_path',type=str,default='./logger_fold_5/log_', help='dir of logger')
    ...
    for fold, (train_idx, test_idx) in enumerate(skf.split(data, data['interaction'])):
        logger.info(f'fold_{fold + 1}')
        train_data = data.iloc[train_idx]
        valid_data, test_data = split_dataset(data.iloc[test_idx], 0.5)
        ...
        test_dataloader = DataLoader(test_dataset, batch_size = args.batch_size, shuffle=False, collate_fn=valid_dataset.collate)
        ...
        tester = Tester(model)
        file_model = './output/model/' + args.dataset + '_'


        logger.info('start training!')
        logger.info(f'{args}')
        logger.info('model10 protein+aff+convtrans')
        best_auc_val = 0.970
        # 初始化
        ema = EMA(model, 0.995)
        ema.register()
        ...
        for epoch in range(1, args.epochs+1):
            ...
            if AUC_val > best_auc_val:
                ...
                tester.save_model(model, file_model, fold, AUC_val)
...
class Tester(object):
    ...
    def save_model(self, model, filename, fold, AUC_val):
        torch.save(model.state_dict(), filename + str(fold) + str(round(AUC_val, 5)) + '.pt')
